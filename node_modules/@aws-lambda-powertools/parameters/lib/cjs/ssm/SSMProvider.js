...
const transformValue_js_1 = require("../base/transformValue.js");
 ...
 */
class SSMProvider extends BaseProvider_js_1.BaseProvider {
    /**
    ...
    async getParametersByName(parameters, options) {
        const configs = {
            ...{
                ...
                maxAge: constants_js_1.DEFAULT_MAX_AGE_SECS,
                throwOnError: true,
            },
            ...options,
        };
    ...
    static handleAnyInvalidGetParameterErrors(result, throwOnError) {
        ...
        if (result.InvalidParameters && result.InvalidParameters.length > 0) {
            if (throwOnError) {
                throw new errors_js_1.GetParameterError(`Failed to fetch parameters: ${result.InvalidParameters.join(', ')}`);
            }
    ...
    static throwIfErrorsKeyIsPresent(parameters, reservedParameter, throwOnError) {
        if (!throwOnError && parameters.hasOwnProperty(reservedParameter)) {
            throw new errors_js_1.GetParameterError(`You cannot fetch a parameter named ${reservedParameter} in graceful error mode.`);
        }
    }
    /**
    ...
    transformAndCacheGetParametersResponse(response, parameters, throwOnError) {
        ...
        for (const parameter of response.Parameters || []) {
            ...
            if (parameterValue && parameterOptions.transform) {
                value = (0, transformValue_js_1.transformValue)(parameterValue, parameterOptions.transform, throwOnError, parameterName);
            }
            ...
            if (value) {
                const cacheKey = [parameterName, parameterOptions.transform].toString();
                this.addToCache(cacheKey, value, parameterOptions.maxAge || constants_js_1.DEFAULT_MAX_AGE_SECS);
            }
