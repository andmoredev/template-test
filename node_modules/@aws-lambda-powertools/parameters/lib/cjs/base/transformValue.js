...
const transformValue = (value, transform, throwOnTransformError, key) => {
    ...
    const isAutoJsonTransform = isAutoTransform && key.toLowerCase().endsWith(`.${constants_js_1.TRANSFORM_METHOD_JSON}`);
    const isAutoBinaryTransform = isAutoTransform &&
        key.toLowerCase().endsWith(`.${constants_js_1.TRANSFORM_METHOD_BINARY}`);
    const isJsonTransform = normalizedTransform === constants_js_1.TRANSFORM_METHOD_JSON;
    const isBinaryTransform = normalizedTransform === constants_js_1.TRANSFORM_METHOD_BINARY;
    ...
    try {
        ...
        if (throwOnTransformError)
            throw new errors_js_1.TransformParameterError(transform, error.message);
        return;
    }
};
